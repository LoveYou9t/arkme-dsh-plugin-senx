...
export function ArkmeFileActions({ block, original }: { block: ArkmeContentBlock; original: ReturnType<typeof useArkmeOriginal> }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (handle !== undefined) {
        ...
        setNotice('已交给浏览器下载，请在浏览器下载列表中查看')
      }
    } catch (error) {
  ...
  return <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', gap: 10, fontSize: 12 }}>
...
export function ArkmeFileViewer({ block, onClose, blocks = [block], onSelect, openLocalFile = false }: {
  ...
  const archive = /\.(zip|rar|7z|tar|gz|bz2|xz)$/i.test(block.fileName)
  ...
  const open = () => {
    if (archive) { setError('压缩文件不支持直接打开，请下载后手动解压'); return }
    setOpenRequested(true); setError('')
    ...
    <div ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={`文件预览 ${block.fileName}`} style={{ position: 'relative', width: showContent ? 'min(860px, 90vw)' : 'min(420px, 90vw)', maxHeight: '80vh', borderRadius: 16, padding: showContent ? '56px 20px 20px' : '48px 40px 32px', color: 'var(--dsw-alias-label-primary, #17191c)', background: 'var(--dsw-alias-bg-elevated, white)' }} onKeyDown={event => {
      ...
      {!showContent ? <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 16 }}>
        ...
        {original.reception.state === 'receiving' && original.localRef === undefined
          ? <div role="status" style={{ width: '100%' }}>正在接收文件<progress aria-label={`接收 ${block.fileName}`} value={original.reception.totalBytes > 0 ? original.reception.receivedBytes : undefined} max={original.reception.totalBytes || 1} style={{ display: 'block', width: '100%', marginTop: 12 }} /></div>
          : <button type="button" onClick={open} disabled={original.localRef === undefined && block.originalRef === undefined} style={{ padding: '10px 24px', border: 0, borderRadius: 8, background: 'var(--dsw-alias-state-business-primary, #3964fe)', color: 'white', fontSize: 14, cursor: 'pointer' }}>{original.localRef === undefined ? '接收文件' : '打开'}</button>}
        {openRequested && original.localRef !== undefined && !browserPreview && !archive && <p role="status">浏览器无法调用本机应用。请使用下方下载按钮，下载后打开。</p>}
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: -56, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
        ...
        <ArkmeFileActions block={block} original={original} />
      </div>
    </div>
  </div>, document.body)
}
