...
function stableDshRpcId(input: Pick<DshRemoteLedgerIdentity, 'bindingRef' | 'runtimeRef' | 'requestRef'>): string {
  return `remote_${createHash('sha256')
    .update(`dsh-remote-rpc-v1\n${input.bindingRef}\n${input.runtimeRef}\n${input.requestRef}`)
...
export class DshRemoteCommandLedger {
  ...
  constructor(
    ...
    this.database.exec(`
      ...
      CREATE TABLE IF NOT EXISTS remote_command_identity (
        ...
        UNIQUE (binding_ref, runtime_ref, request_ref)
      );
      CREATE TABLE IF NOT EXISTS remote_command_event (
        event_id INTEGER PRIMARY KEY AUTOINCREMENT,
        identity_id INTEGER NOT NULL REFERENCES remote_command_identity(identity_id) ON DELETE CASCADE,
        state TEXT NOT NULL CHECK (state IN ('pending', 'completed', 'outcome_unknown')),
        nonce TEXT NOT NULL,
        ciphertext TEXT NOT NULL,
        created_at_millis INTEGER NOT NULL
      );
      CREATE INDEX IF NOT EXISTS remote_command_event_identity
        ON remote_command_event (identity_id, event_id DESC);
      CREATE INDEX IF NOT EXISTS remote_command_identity_created
        ON remote_command_identity (created_at_millis DESC);
    `)
    this.secureFiles()
  }
  ...
  begin(input: DshRemoteLedgerIdentity): { duplicate: boolean; entry: DshRemoteLedgerEntry } {
    ...
    this.transaction(() => {
      const result = this.database.prepare(`
        INSERT INTO remote_command_identity (
          account_id, binding_ref, runtime_ref, request_ref, operation, arguments_hash,
          dsh_rpc_id, execute_before_millis, created_at_millis
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(
        input.accountId, input.bindingRef, input.runtimeRef, input.requestRef, input.operation, hash,
  ...
  complete(
    identity: Pick<DshRemoteLedgerIdentity, 'accountId' | 'bindingRef' | 'runtimeRef' | 'requestRef'>,
  ...
  get(identity: Pick<DshRemoteLedgerIdentity, 'accountId' | 'bindingRef' | 'runtimeRef' | 'requestRef'>): DshRemoteLedgerEntry | undefined {
  ...
  pending(accountId: string, nowMillis = this.now()): DshRemoteLedgerEntry[] {
    const rows = this.database.prepare(`
      SELECT identity_id, account_id, binding_ref, runtime_ref, request_ref, operation,
             arguments_hash, dsh_rpc_id, execute_before_millis, created_at_millis
      FROM remote_command_identity
      WHERE account_id = ?
  ...
  cleanup(options: { retentionMillis?: number; maxCommands?: number } = {}): number {
    ...
    const before = Number((this.database.prepare('SELECT COUNT(*) AS count FROM remote_command_identity').get() as { count: number }).count)
    this.transaction(() => {
      this.database.prepare('DELETE FROM remote_command_identity WHERE created_at_millis < ?').run(cutoff)
      this.database.prepare(`
        DELETE FROM remote_command_identity
        WHERE identity_id NOT IN (
          SELECT identity_id FROM remote_command_identity ORDER BY created_at_millis DESC LIMIT ?
        )
      `).run(maxCommands)
    })
    const after = Number((this.database.prepare('SELECT COUNT(*) AS count FROM remote_command_identity').get() as { count: number }).count)
    this.secureFiles()
    return before - after
  }
  ...
  private validateIdentity(input: DshRemoteLedgerIdentity): void {
    for (const [name, value] of Object.entries({
      accountId: input.accountId, bindingRef: input.bindingRef, runtimeRef: input.runtimeRef, requestRef: input.requestRef,
    })) {
  ...
  private identity(input: Pick<DshRemoteLedgerIdentity, 'bindingRef' | 'runtimeRef' | 'requestRef'>): IdentityRow | undefined {
    return this.database.prepare(`
      SELECT identity_id, account_id, binding_ref, runtime_ref, request_ref, operation,
             arguments_hash, dsh_rpc_id, execute_before_millis, created_at_millis
      FROM remote_command_identity
      WHERE binding_ref = ? AND runtime_ref = ? AND request_ref = ?
    `).get(input.bindingRef, input.runtimeRef, input.requestRef) as unknown as IdentityRow | undefined
  }

  private requireIdentity(input: Pick<DshRemoteLedgerIdentity, 'accountId' | 'bindingRef' | 'runtimeRef' | 'requestRef'>): IdentityRow {
    const row = this.identity(input)
  ...
  private entry(row: IdentityRow): DshRemoteLedgerEntry {
    const event = this.database.prepare(`
      ...
      FROM remote_command_event WHERE identity_id = ? ORDER BY event_id DESC LIMIT 1
  ...
  private appendEvent(identityId: number, state: DshRemoteLedgerState, payload: Record<string, unknown>, now: number): void {
    const row = this.database.prepare(`
      SELECT identity_id, account_id, binding_ref, runtime_ref, request_ref, operation,
             arguments_hash, dsh_rpc_id, execute_before_millis, created_at_millis
      FROM remote_command_identity WHERE identity_id = ?
    ...
    this.database.prepare(`
      INSERT INTO remote_command_event (identity_id, state, nonce, ciphertext, created_at_millis)
      VALUES (?, ?, ?, ?, ?)
  ...
  private eventAad(row: IdentityRow, state: DshRemoteLedgerState, now: number): string {
    return canonicalJson({
      schemaVersion: 1, identityId: row.identity_id, accountId: row.account_id,
      bindingRef: row.binding_ref, runtimeRef: row.runtime_ref, requestRef: row.request_ref,
