...
export type RecordEditHistoryTarget = { viewerUserId: number; recordUid: string } & (
  | { kind: 'owned' }
  | { kind: 'chat'; chatSessionUid: string; relationUid: string }
)

export interface RecordRevisionContentProjector {
  project(snapshot: Record<string, unknown>, viewerUserId: number): ArkmeRecordRevision['content']
}
...
export class RecordEditHistoryService {
  ...
  async page(target: RecordEditHistoryTarget, cursorEditAt = 0, signal?: AbortSignal): Promise<ArkmeRecordEditHistoryPage> {
    ...
    const window = { limit: 50, include_media: true, ...(cursorEditAt === 0 ? {} : { cursor_edit_at: cursorEditAt }) }
    ...
    const items = data.items.flatMap((raw): ArkmeRecordRevision[] => {
      ...
      return [{ revisionUid: revision.revision_uid, kind: revision.revision_type === 5 ? 'original' : 'manual',
        editAtMillis: revision.edit_at, content: this.projector.project(revision, session.userId) }]
    })
