...
it('shares authorized pages, presentation and cache across Host, SDK and official DSH ToolRuntime', async () => {
  ...
  const fetchImpl: typeof fetch = async (input, init) => {
    ...
    if (endpoint.endsWith('/members/page')) {
    ...
    } else if (endpoint.endsWith('/members/by-user-ids')) data = { chat_session_uid: body.chat_session_uid, items: active ? [{ user_id: 2, status: 1, role: 3, remark: '私人备注', display_name_snapshot: '群内昵称', display_name: '公开昵称', join_at: 1 }] : [] }
      ...
      items: [{ user_id: 2, read_status: 'unread', read_at: 0 }] }
  ...
  try {
    ...
    expect(first.presentationComplete).toBe(false)
    ...
    unsupported = true
    runtime.invalidateScope(runtime.requestScope(1))
    await expect(chat.pageSourceMembers(group.sourceRef)).rejects.toMatchObject({ code: 'member-pagination-unavailable' })
    unsupported = false
    runtime.invalidateScope(runtime.requestScope(1))
