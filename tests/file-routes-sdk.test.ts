...
describe('file Host and external SDK transport contract', () => {
  it('stages, reads ranges, queues sends, recovers tasks, and enforces account/origin boundaries', async () => {
    ...
    const owner = new FileTransfers(directory, { currentUser: async () => userId, validateSource: async () => {}, upload, send, fetchMedia: async () => { throw new Error('not expected') } }, 1000)
    const facade = {
    ...
    try {
      ...
      expect((await sdk.localFiles()).map(value => value.fileRef)).toEqual([html.fileRef])
    } finally {
