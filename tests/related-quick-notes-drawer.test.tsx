...
describe('normal timeline related quick note drawer', () => {
  it('opens edited history only for manual edits and returns to the detail', async () => {
    ...
    expect(view.root.findAllByType('button').some(node => node.children.includes('已编辑 ›'))).toBe(false)
    ...
    await act(async () => view.root.findAllByType('button').find(node => node.children.includes('已编辑 ›'))!.props.onClick())
