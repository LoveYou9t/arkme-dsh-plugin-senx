...
function fixture(response: unknown) {
  ...
  const projector = { project: vi.fn((raw: Record<string, unknown>) => ({ textContent: raw.text_content, contentBlocks: raw.media_display_items ?? [] })) }
...
describe('Record edit history', () => {
  ...
  it('reads owned history, preserves revision identity and renders only manual/original snapshots', async () => {
    ...
    expect(f.runtime.authenticatedPost).toHaveBeenCalledWith('/api/v1/records/revisions/query', { record_uid: 'r1', limit: 50, include_media: true }, { userId: 42 }, undefined)
  ...
  it('routes chat through its relation gate without falling back to owned reads', async () => {
    ...
    expect(await f.service.page({ kind: 'chat', viewerUserId: 42, recordUid: 'r1', chatSessionUid: 's1', relationUid: 'rel1' }, 200)).toEqual({ items: [], hasMore: true, nextCursorEditAt: 100 })
    expect(f.runtime.authenticatedChatPost).toHaveBeenCalledWith('/api/v1/chats/records/revisions/query', { chat_session_uid: 's1', rel_uid: 'rel1', limit: 50, include_media: true, cursor_edit_at: 200 }, { userId: 42 }, undefined)
  ...
  it('passes only the historical snapshot and never fetches the current record media', async () => {
    ...
    expect(f.projector.project).toHaveBeenCalledWith(historic, 42)
  })
  ...
  it('rejects a response after account changes or cancellation during the request', async () => {
    ...
    expect(f.projector.project).not.toHaveBeenCalled()
    ...
    expect(aborted.projector.project).not.toHaveBeenCalled()
  })
